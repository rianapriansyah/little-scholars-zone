-- Families aren't identified by a "family name" the way Western households are — Indonesian
-- families are commonly referred to by a parent's own name instead. Drop the field; the admin
-- UI now derives a display name from father_name, falling back to mother_name.
